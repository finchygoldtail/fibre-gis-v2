...
export function useLiveUserLocationSharing(args: {
  ...
  useEffect(() => {
    ...
    const writePosition = (position: GeolocationPosition) => {
      ...
      void upsertLiveUserLocation({
        ...
        setShareError("Could not share location. Check permissions.");
      });
    };

    const handleError = () => {
      setShareError("Could not get GPS location. Check browser permissions.");
      setSharingEnabled(false);
    };
  ...
  useEffect(() => {
    ...
    void clearLiveUserLocation(businessId, uid).catch((error) => {
  ...
  useEffect(() => {
    return () => {
      ...
      if (uid) {
        void clearLiveUserLocation(businessId, uid);
      }
    };
  }, [businessId, uid]);
