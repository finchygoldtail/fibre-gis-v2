...
function getCableLayerText(asset: SavedMapAsset): string {
  return getAssetClassificationText(asset);
}
  ...
  const cableAssets = useMemo(
    () =>
      assets.filter((asset) => {
        ...
        if (!isCableAsset(asset) || asset.geometry?.type !== "LineString" || !Array.isArray(asset.geometry.coordinates)) {
          return false;
        }
        ...
        const layerKind = getCableLayerKind(asset);
        if (!layerKind) return false;
        if (layerKind === "drop") return isLayerOn("dropCables");

        const layerKeyByKind: Partial<Record<NonNullable<typeof layerKind>, string>> = {
          feeder: "feeders",
          link: "links",
          ulw96: "ulw96",
          ulw48: "ulw48",
        ...
        const layerKey = layerKeyByKind[layerKind];

        if (layerKey && !isLayerOn(layerKey)) return false;

        if (allCableLayersVisible) return true;

        return Boolean(layerKey && isLayerOn(layerKey));
      }),
  ...
  return (
    <>
      {cableAssets.map((asset) => {
        ...
        const cableColor = isPostgisHighlighted ? "#06b6d4" : isSelected ? "#f59e0b" : baseColor;
        ...
        return (
          <React.Fragment key={asset.id}>
            <Polyline
              ...
              pathOptions={{
                color: cableColor,
                weight: isPostgisHighlighted ? 7 : isSelected ? 9 : isHovered || isEditing ? 7 : 4,
                opacity: isPostgisHighlighted || isSelected || isHovered || isEditing ? 1 : 0.85,
                dashArray: isPostgisHighlighted ? "12, 8" : getDashArray(asset),
