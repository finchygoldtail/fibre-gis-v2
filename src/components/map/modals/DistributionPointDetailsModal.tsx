...
function getCableDisplaySize(cable?: SavedMapAsset | null): string {
  const total = getCableFibreTotalFromAsset(cable);
  return total > 0 ? `${total}F` : "Unknown size";
}
...
function isRouteSupportingCableCandidate(asset: SavedMapAsset | null | undefined): boolean {
  ...
  // Drop/home tails are not valid upstream/supporting route cables for AFN/MDU routing.
  if (haystack.includes("drop")) return false;
...
function upsertSbToSbRoute(
  ...
  return {
    ...
    afnDetails: {
      ...
      splitterOutputs: 8,
      throughCableId:
        nextRoute.supportingCableId || details.afnDetails?.throughCableId,
    },
...
function buildParentSbReservationView(args: {
  ...
  const endpointParent = findAssetByAnyRef(
    [
      ...
      return (
        valueRef && !childRefs.some((childRef) => refsMatch(valueRef, childRef))
      );
    }),
    ...
    !endpointParent && line.length && childPoint
      ? [...allDistributionPoints, ...allAssets]
          ...
          .filter((candidate) => {
            ...
            return (
              !refsMatch(candidateTitle, childName) &&
              !refsMatch((candidate as any).id, activeDpAsset?.id)
            );
          })
          .map((candidate) => {
            ...
            const childDistanceToEnd = distanceMeters(
              childPoint,
              line[line.length - 1],
            );
  ...
  const parentName = parentAsset
    ? getAssetTitle(parentAsset)
    : text(cableItem.parentSbName || cableItem.parentDpName || "Parent SB");

  const mappings =
    parentFibres.length === localFibres.length
      ? parentFibres.map((parent, index) => ({
          parent,
  ...
  return {
    ...
    parentCableName: text(
      cableItem.parentCableName || cableItem.parentCableId || "",
    ),
    mappings,
  };
}
...
export default function DistributionPointDetailsModal({
    ...
    return (
      [...allDistributionPoints, ...allAssets].find((asset: any) => {
        ...
        return keys.some((key) =>
          lookupKeys.some((lookup) => refsMatch(key, lookup)),
        );
      }) || null
    );
  ...
  const autoDetectedCable = useMemo(() => {
    if (selectedCableFromId || selectedCableId || !activeDpAsset) return null;

    const dpPoint = getPoint(activeDpAsset);
    if (!dpPoint) return null;

    const uniqueCables = new Map<string, SavedMapAsset>();
    [...availableThroughCables, ...allAssets]
      .filter(isRouteSupportingCableCandidate)
      .forEach((cable: any) => {
        const id = text(
          cable.id || cable.assetId || cable.cableId || cable.name,
        );
        if (id && !uniqueCables.has(id)) uniqueCables.set(id, cable);
      });

    const matches = Array.from(uniqueCables.values())
      .map((cable) => {
        const line = getLinePoints(cable);
        if (!line.length) return null;

        const distance = Math.min(
          ...line.map((point) => distanceMeters(dpPoint, point)),
        );

        return { cable, distance };
      })
      .filter((item): item is { cable: SavedMapAsset; distance: number } =>
  ...
  const selectedCable = selectedCableFromId || autoDetectedCable || undefined;
      ...
      [...allDistributionPoints, ...allAssets]
        ...
        .filter((asset, index, arr) => {
          ...
          return (
            arr.findIndex(
              (candidate: any) =>
                normaliseRef(candidate?.id || getAssetTitle(candidate)) === id,
            ) === index
          );
        })
  ...
  const [manualParentFibres, setManualParentFibres] = useState<string>(
    formatFibreSelection(
      primarySbRoute?.parentFibres ||
        (details as any)?.afnDetails?.parentInputFibres ||
        [],
    ),
  );
  const [manualLocalFibres, setManualLocalFibres] = useState<string>(
    formatFibreSelection(
      primarySbRoute?.localFibres || details.afnDetails?.inputFibres || [],
    ),
  );
  useEffect(() => {
    ...
    setManualParentFibres(
      formatFibreSelection(
        nextPrimaryRoute?.parentFibres ||
          (details as any)?.afnDetails?.parentInputFibres ||
          [],
      ),
    );
    setManualLocalFibres(
      formatFibreSelection(
        nextPrimaryRoute?.localFibres || details.afnDetails?.inputFibres || [],
      ),
    );
    setManualSbNote(nextPrimaryRoute?.note || "");
  }, [activeDpId, details]);

  const [manualSbNote, setManualSbNote] = useState<string>(
    primarySbRoute?.note || "",
  );
  ...
  function applyManualSbRoute() {
    ...
    if (!parentFibres.length && !localFibres.length) {
      alert(
        "Enter the fibres used for this SB to SB route, for example 1-4 or 7, 8, 9.",
      );
      return;
    }

    const normalisedLocalFibres = localFibres.length
      ? localFibres
      : parentFibres;
    const nextRoute: SbToSbFibreRoute = {
      id: makeSbRouteId(
        (fromSb as any).id || getAssetTitle(fromSb),
        (toSb as any).id || getAssetTitle(toSb),
      ),
      fromSbId: text(
        (fromSb as any).id || (fromSb as any).assetId || getAssetTitle(fromSb),
      ),
      fromSbName: getAssetTitle(fromSb),
      toSbId: text(
        (toSb as any).id || (toSb as any).assetId || getAssetTitle(toSb),
      ),
      toSbName: getAssetTitle(toSb),
      parentFibres,
      localFibres: normalisedLocalFibres,
      supportingCableId: selectedCableId || undefined,
      supportingCableName: selectedCable
        ? getAssetTitle(selectedCable)
        : undefined,
  ...
  const jointMatchedDpState = useMemo(() => {
    ...
    return (Object.values(networkState.dpStates || {}).find((state: any) =>
      lookupKeys.some((key) =>
  ...
  const activeDpDetails = ((activeDpAsset as any)?.dpDetails ||
  ...
  const capacity = isAfn
    ...
    : Number(
        details.connectionsToHomes || activeDpDetails.connectionsToHomes || 0,
      );
  const used = connectedHomes.length;
  const available = Math.max(0, capacity - used);
  const operationalCapacityPercent =
  ...
  const fibreTotal = getCableFibreTotalFromAsset(selectedCable);
  const selectedInputFibreCount = effectiveInputFibres.length;
  const passthroughFibreCount = selectedCable
  ...
  const handleSave = async () => {
    try {
      ...
      const nextDetails = removeUndefinedDeep({ ...details, image: imageUrl });
  ...
  return (
    <>
      ...
      <div className="modal">
        ...
        {isAfn ? (
          <div className="afn-panel">
            ...
            <div className="sb-route-builder">
              <div className="parent-sb-title">
                Manual SB → SB Fibre Reservation
              </div>
                ...
                <label>
                  ...
                  >
                    ...
                    {sbRouteTargets
                      .filter(
                        (dp: any) =>
                          !refsMatch(dp.id || getAssetTitle(dp), manualToSbId),
                      )
                      .map((dp: any) => (
                        <option
                          key={dp.id || getAssetTitle(dp)}
                          value={dp.id || getAssetTitle(dp)}
                        >
                          {getAssetTitle(dp)}
                        </option>
                      ))}
              ...
              {storedSbRoutes.length ? (
                <div className="sb-route-list">
                  {storedSbRoutes.map((route) => (
                    <div key={route.id} className="sb-route-item">
                      <strong>
                        {route.fromSbName} → {route.toSbName}
                      </strong>
                      <span>
                        Parent fibres:{" "}
                        {formatFibreSelection(route.parentFibres) || "—"}
                        {" · "}
                        Local fibres:{" "}
                        {formatFibreSelection(route.localFibres) || "—"}
                      </span>
                    </div>
                  ))}
                </div>
                ...
                <div className="parent-sb-note">
                  This directly reserves fibres from one SB to another. Cable
                  detection and joint uploads cannot overwrite this route.
                </div>
              )}
            </div>
              ...
              <div className="parent-sb-reservation">
                ...
                <div className="parent-sb-path">
                  {dpRelationshipRouting.selectedParent ? (
                    <>
                      <strong>
                        {dpRelationshipRouting.selectedParent.name}
                      </strong>
                      <span>→</span>
                ...
                {dpRelationshipRouting.mapping.length ? (
                  <div className="parent-sb-map">
                    {dpRelationshipRouting.mapping.map((mapping) => (
                      <span
                        key={`${mapping.parentFibre}-${mapping.localFibre}`}
                      >
            ...
            {parentSbReservationView ? (
              <div className="parent-sb-reservation">
                ...
                <div className="parent-sb-fibres">
                  ...
                  {parentSbReservationView.requiredFibres === 1 ? "" : "s"}{" "}
                  needed
                </div>
            ...
            >
              ...
              {availableThroughCables.map((cable) => (
                <option key={cable.id} value={cable.id}>
                  {cable.name || cable.id} — {getCableDisplaySize(cable)}
                </option>
              ))}
            </select>
            ...
            {selectedCableId ? (
              <div className="afn-readonly-fibres">
                <div className="afn-grid-header">
                  ...
                  <em>
                    {effectiveInputFibres.length} shown · {capacity || 0}{" "}
                    outputs
                  </em>
                </div>
                ...
                <div className="afn-summary">
                  ...
                  are displayed here for reference only and are no longer
                  manually selected from this DP modal.
                </div>
                ...
                {effectiveInputFibres.length ? (
                  ...
                  <div className="afn-summary">
                    No local splitter fibres are stored on this DP yet. Upload
                    or rebuild from joint continuity to populate the routing
                    view.
                  </div>
                )}
              </div>
              ...
              <div className="afn-summary">
                Select a supporting cable to view the joint-controlled fibre
                source.
              </div>
            )}
        ...
        <select
          value={isAfn ? capacity : details.connectionsToHomes || 8}
          disabled={isAfn}
          onChange={(e) => update("connectionsToHomes", Number(e.target.value))}
        >
          {isAfn ? (
            <option value={capacity}>
              {capacity} from selected AFN fibres
            </option>
          ) : null}
        ...
        <div className="afn-summary">
          ...
          {selectedCable?.name ||
            ...
            "not selected"}
          <br />
          Input fibres consumed:{" "}
          {selectedInputFibreCount ||
            details.mduDetails?.totalReservedFibres ||
            0}
          <br />
          Passthrough fibres: {selectedCable ? passthroughFibreCount : "—"}
          <br />
          Capacity state: {operationalCapacityWarning} (
          {operationalCapacityPercent}%)
        </div>
