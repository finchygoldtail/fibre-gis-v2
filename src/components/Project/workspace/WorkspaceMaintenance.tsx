...
export default function WorkspaceMaintenance({ auditIssues, stats }: Props) {
  return <>
    <section style={panel}><h3 style={title}>Maintenance Queue</h3><Tile label="Open Issues" value={n(auditIssues?.length ?? stats?.issueCount)} /></section>
    <section style={wide}><h3 style={title}>Maintenance Notes</h3><p style={{ color: "#cbd5e1" }}>This panel is ready for maintenance logs, photo status, OTDR evidence and defect close-out. It does not touch the existing maintenance log storage yet.</p></section>
  </>;
}
