...
function isWorkspaceHomeAssetForLayerCount(asset: SavedMapAsset | null | undefined): boolean {
  if (!asset || isHomeDropCableAsset(asset) || isWorkspaceDistributionPointAsset(asset) || isDesignCableAsset(asset)) return false;
...
function isDropCableLinkedToHome(drop: SavedMapAsset, home: SavedMapAsset): boolean {
...
function getWorkspaceHomeConnectionStatus(
  ...
  const dropStatus = normaliseWorkspaceHomeStatus((drop as any).customerStatus || (drop as any).homeStatus || (drop as any).status);
...
const homesLive = Math.min(
  ...
  const workspaceLayerCounts = useMemo(() => {
    ...
    const connectedHomes = canonicalHomes.filter(
      (asset) => getWorkspaceHomeConnectionStatus(asset, workspaceAssets) === "connected",
    );
    const unconnectedHomes = canonicalHomes.filter(
      (asset) => getWorkspaceHomeConnectionStatus(asset, workspaceAssets) === "unconnected",
    );
    const liveHomes = canonicalHomes.filter(
      (asset) => getWorkspaceHomeConnectionStatus(asset, workspaceAssets) === "live",
    );

    const areaAssets = workspaceAssets.filter((asset) => asset.geometry?.type === "Polygon" && asset.id !== projectArea?.id);
    const designCableAssets = workspaceAssets.filter(isDesignCableAsset);
    const dropCableAssets = workspaceAssets.filter(isHomeDropCableAsset);

    const hasPointGeometry = (asset: SavedMapAsset) => {
      const item = asset as any;
      return asset.geometry?.type === "Point" || (typeof item.lat === "number" && typeof item.lng === "number");
    };

    const jointAssets = workspaceAssets.filter((asset) => {
      if (!hasPointGeometry(asset) || isWorkspaceDistributionPointAsset(asset)) return false;
    ...
    const poleAssets = workspaceAssets.filter((asset) => hasPointGeometry(asset) && getWorkspaceAssetLayerText(asset).includes("pole"));
  ...
  const openreachLayerCounts = useMemo(() => {
    ...
    return {
      ducts: openreachAssets.filter((asset) => asset.geometry?.type === "LineString" && !getWorkspaceAssetLayerText(asset).includes("trench") && !getWorkspaceAssetLayerText(asset).includes("span")).length,
      trenches: openreachAssets.filter((asset) => getWorkspaceAssetLayerText(asset).includes("trench")).length,
      spans: openreachAssets.filter((asset) => getWorkspaceAssetLayerText(asset).includes("span") || getWorkspaceAssetLayerText(asset).includes("overhead")).length,
      chambers: openreachAssets.filter((asset) => {
        const text = getWorkspaceAssetLayerText(asset);
        return text.includes("chamber") || text.includes("manhole") || text.includes("jc:") || text.includes("ch:");
      }).length,
      poles: openreachAssets.filter((asset) => getWorkspaceAssetLayerText(asset).includes("pole") || getWorkspaceAssetLayerText(asset).includes("pol:")).length,
      labels: openreachAssets.filter((asset) => asset.geometry?.type === "LineString").length,
                  ...
                  <div style={layerMenu}>
                    ...
                    {workspaceLayerOptions.map((layer) => (
                      <label key={layer.key} style={layerRow}>
                        ...
                        />
                        <span>{formatLayerLabelWithCount(layer.label, workspaceLayerCounts[layer.key])}</span>
                      </label>
                    ))}
                    ...
                    {openreachLayerOptions.map((layer) => (
                      <label key={layer.key} style={layerRow}>
                        ...
                        />
                        <span>{formatLayerLabelWithCount(layer.label, openreachLayerCounts[layer.key])}</span>
                      </label>
                    ))}
