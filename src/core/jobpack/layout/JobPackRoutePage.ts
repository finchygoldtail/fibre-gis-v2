...
export function fibreBucket(feature: JobPackMapFeature): '96F' | '48F' | '36F' | '24F' | '12F' | 'DROP' | 'OTHER' {
  const text = routeText(feature);
  const source = (feature.asset.sourceAsset || {}) as any;
  if (
    text.includes('drop') ||
    text.includes('spur') ||
    ...
    Boolean(source?.homeId || source?.connectedHomeId || source?.toHomeId || source?.fromHomeId || source?.properties?.homeId || source?.properties?.connectedHomeId || source?.properties?.toHomeId || source?.properties?.fromHomeId)
  ) return 'DROP';
  if (text.includes('96f') || /\b96\b/.test(text)) return '96F';
  if (text.includes('48f') || /\b48\b/.test(text)) return '48F';
  if (text.includes('36f') || /\b36\b/.test(text)) return '36F';
  if (text.includes('24f') || /\b24\b/.test(text)) return '24F';
...
function strokeFor(feature: JobPackMapFeature, focused = false): { stroke: string; width: number; dash: string; className: string } {
  ...
  if (bucket === '36F') return { stroke: '#0ea5e9', width: focused ? 7.5 : 4.2, dash: '14 9', className: 'route-36' };
  if (bucket === '24F') return { stroke: '#f59e0b', width: focused ? 7 : 4, dash: '13 8', className: 'route-24' };
