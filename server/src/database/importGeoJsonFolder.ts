import fs from "node:fs/promises";
import path from "node:path";
import { closeDatabasePool } from "../config/database.js";
import { importGeoJsonAssets, type ImportAssetsResult } from "../services/assetImportService.js";
import type { GeoJsonFeature } from "../types/geojson.js";

type CliArgs = {
  dir: string;
  businessId: string;
  projectId?: string;
  source: string;
  sourceRevision?: string;
  dryRun: boolean;
};

type FileImportSummary = ImportAssetsResult & {
  file: string;
  areaId: string;
};

async function importGeoJsonFolder(): Promise<void> {
  const args = parseArgs(process.argv.slice(2));
  const dirPath = path.resolve(args.dir);
  const entries = await fs.readdir(dirPath, { withFileTypes: true });
  const files = entries
    .filter((entry) => entry.isFile() && entry.name.toLowerCase().endsWith(".geojson"))
    .map((entry) => entry.name)
    .sort();

  if (files.length === 0) {
    throw new Error(`No .geojson files found in ${dirPath}`);
  }

  const results: FileImportSummary[] = [];

  for (const file of files) {
    const filePath = path.join(dirPath, file);
    const raw = await fs.readFile(filePath, "utf8");
    const parsed = JSON.parse(raw);
    const features = getFeatures(parsed);
    const areaId = areaIdFromFilename(file);

    const result = await importGeoJsonAssets({
      businessId: args.businessId,
      projectId: args.projectId,
      areaId,
      source: args.source,
      sourceRevision: args.sourceRevision,
      sourceFile: file,
      dryRun: args.dryRun,
      features,
    });

    results.push({
      file,
      areaId,
      ...result,
    });
  }

  const total = results.reduce(
    (summary, result) => {
      summary.read += result.read;
      summary.valid += result.valid;
      summary.insertedOrUpdated += result.insertedOrUpdated;
      summary.skipped += result.skipped;

      Object.entries(result.byType).forEach(([type, count]) => {
        summary.byType[type] = (summary.byType[type] || 0) + count;
      });

      return summary;
    },
    {
      read: 0,
      valid: 0,
      insertedOrUpdated: 0,
      skipped: 0,
      byType: {} as Record<string, number>,
    },
  );

  console.log(JSON.stringify(
    {
      directory: dirPath,
      files: results.length,
      dryRun: args.dryRun,
      total,
      results,
    },
    null,
    2,
  ));
}

function areaIdFromFilename(file: string): string {
  return path
    .basename(file, path.extname(file))
    .trim()
    .replace(/[^a-zA-Z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "")
    .toUpperCase();
}

function getFeatures(parsed: unknown): GeoJsonFeature[] {
  const item = parsed as any;
  if (item?.type === "FeatureCollection" && Array.isArray(item.features)) {
    return item.features as GeoJsonFeature[];
  }
  if (item?.type === "Feature") {
    return [item as GeoJsonFeature];
  }
  if (Array.isArray(item)) {
    return item as GeoJsonFeature[];
  }
  throw new Error("Expected GeoJSON FeatureCollection, Feature, or Feature array");
}

function parseArgs(argv: string[]): CliArgs {
  const values = new Map<string, string | boolean>();

  for (let index = 0; index < argv.length; index += 1) {
    const arg = argv[index];
    if (!arg.startsWith("--")) continue;

    const key = arg.slice(2);
    if (key === "dry-run") {
      values.set(key, true);
      continue;
    }

    const value = argv[index + 1];
    if (!value || value.startsWith("--")) {
      throw new Error(`Missing value for --${key}`);
    }
    values.set(key, value);
    index += 1;
  }

  return {
    dir: getRequired(values, "dir"),
    businessId: getRequired(values, "business-id"),
    projectId: getOptional(values, "project-id"),
    source: getOptional(values, "source") || "geojson-folder-import",
    sourceRevision: getOptional(values, "source-revision"),
    dryRun: values.get("dry-run") === true,
  };
}

function getRequired(values: Map<string, string | boolean>, key: string): string {
  const value = getOptional(values, key);
  if (!value) throw new Error(`--${key} is required`);
  return value;
}

function getOptional(values: Map<string, string | boolean>, key: string): string | undefined {
  const value = values.get(key);
  return typeof value === "string" && value.trim() ? value.trim() : undefined;
}

importGeoJsonFolder()
  .then(async () => {
    await closeDatabasePool();
  })
  .catch(async (err) => {
    console.error("GeoJSON folder import failed", err);
    await closeDatabasePool();
    process.exit(1);
  });
